      ...
      {showCapturePreview && (
        <div className="fixed inset-0 z-[60] bg-slate-950 flex flex-col p-6 overflow-y-auto">
          <div className="max-w-md mx-auto w-full flex-1 flex flex-col py-10">
            <h2 className="text-white text-2xl font-black text-center mb-8 italic uppercase tracking-tighter">Review Bon</h2>
      ...
      <header className="bg-white px-6 pt-12 pb-6 border-b border-slate-100 sticky top-0 z-30">
        <div className="flex justify-between items-end max-w-2xl mx-auto">
          ...
          <div className="text-right">
            <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Bulan Ini</p>
                ...
                <div className="bg-slate-50 p-2 rounded-3xl border border-slate-100">
                   <div className="max-h-60 overflow-y-auto no-scrollbar space-y-2 p-1">
                      {scannedData.items?.map((item, idx) => (
                        <div key={idx} className="bg-white p-3 rounded-2xl shadow-sm relative">
                           ...
                           <div className="grid grid-cols-3 gap-2">
                              <div>
                                <span className="text-[8px] font-bold text-slate-300 block">Qty</span> 
                                <input 
                                  type="number" 
                              ...
                              <div>
                                <span className="text-[8px] font-bold text-slate-300 block">Rp/u</span> 
                                <input 
                                  type="number" 
                              ...
                              <div className="text-right">
                                <span className="text-[8px] font-bold text-slate-300 block">Subtotal</span>
                                <span className="text-[10px] font-black text-blue-600">Rp {item.total.toLocaleString()}</span>
                              </div>
                           </div>
                           <button onClick={() => {
                             const newItems = scannedData.items?.filter((_, i) => i !== idx);
                             setScannedData({...scannedData, items: newItems});
                           }} className="absolute top-2 right-2 text-red-200"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" strokeWidth="3"/></svg></button>
                        </div>
                      ))}
                   </div>
                   <button onClick={() => {
                  ...
                  {expandedReceiptId === receipt.id && (
                    <div className="px-5 pb-5 pt-1 bg-slate-50/50 space-y-2">
                      {receipt.items.map((item, i) => (
                        <div key={i} onClick={(e) => { e.stopPropagation(); checkPrice(item.name); }} className="flex justify-between p-3 bg-white rounded-2xl shadow-sm border border-slate-50">
